...
#include "compute_pair_local_mol.h"
...
enum{DIST,ENG,FORCE,FX,FY,FZ,PN,ID1,ID2,MOL1,MOL2};
enum{TYPE,RADIUS};

/* ---------------------------------------------------------------------- */

ComputePairLocalMol::ComputePairLocalMol(LAMMPS *lmp, int narg, char **arg) :
  ...
  pstyle(nullptr), pindex(nullptr), vlocal(nullptr), alocal(nullptr)
{
  ...
  while (iarg < narg) {
    ...
    else if (strcmp(arg[iarg],"id1") == 0) pstyle[nvalues++] = ID1;
    else if (strcmp(arg[iarg],"id2") == 0) pstyle[nvalues++] = ID2;
    else if (strcmp(arg[iarg],"mol1") == 0) pstyle[nvalues++] = MOL1;
    else if (strcmp(arg[iarg],"mol2") == 0) pstyle[nvalues++] = MOL2;
...
ComputePairLocalMol::~ComputePairLocalMol()
{
...
void ComputePairLocalMol::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void ComputePairLocalMol::compute_local()
{
...
int ComputePairLocalMol::compute_pairs(int flag)
{
...
void ComputePairLocalMol::reallocate(int n)
{
...
double ComputePairLocalMol::memory_usage()
{
