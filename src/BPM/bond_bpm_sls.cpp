...
BondBPMSLS::BondBPMSLS(LAMMPS *_lmp) :
    ...
    aT(nullptr), aT_temp(nullptr), id_fix_property_bond(nullptr)
{
  ...
  update_flag = 1;
  id_fix_bond_history = utils::strdup("HISTORY_BPM_SLS");
...
double BondBPMSLS::store_bond(int n, int i, int j)
{
  int type;
  ...
  if (i < atom->nlocal) {
    for (int m = 0; m < atom->num_bond[i]; m++) {
      if (atom->bond_atom[i][m] == tag[j]) { 
        ...
        fix_bond_history->update_atom_value(i, m, 2, 0); // reserved for future use
  ...
  if (j < atom->nlocal) {
    for (int m = 0; m < atom->num_bond[j]; m++) {
      if (atom->bond_atom[j][m] == tag[i]) { 
        ...
        fix_bond_history->update_atom_value(j, m, 2, 0); //reserved for future use
...
void BondBPMSLS::store_data()
{
  ...
  for (i = 0; i < atom->nlocal; i++) {
    for (m = 0; m < atom->num_bond[i]; m++) {
      ...
      fix_bond_history->update_atom_value(i, m, 0, r); // r0
      fix_bond_history->update_atom_value(i, m, 1, r); // rn
      fix_bond_history->update_atom_value(i, m, 2, 0); // reserved for use
...
void BondBPMSLS::compute(int eflag, int vflag)
{
  ...
  double k1, eta1, exp_j, Hn1, Hn2, termA1, termA2, termB1, termB2;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    // k0, k1, eta1, h1, h2 , taum
    k1 = tb->kfile[0];
    eta1 = aT[type] * tb->etafile[0];
    exp_j = tb->expfile[0];
    Hn1 = bondstore[n][2];
    Hn2 = bondstore[n][3];

    // Steady-state solution t->inf
    fbond = (k0[type] * k1)/(k0[type] + k1) * (r0 - r);

    // compute updated h1 and h2
    termA1 = exp_j * Hn1;
    termA2 = k0[type] * (rn - r) * (1 - exp_j) / (dt * (k0[type] + k1 / eta1));

    termB1 = exp_j * Hn2;
    termB2 = (k0[type] * k1)/(k0[type] + k1) * (rn - r) * (1 - exp_j) / (dt * (k0[type] + k1 / eta1));
...
void BondBPMSLS::coeff(int narg, char **arg)
{
...
void BondBPMSLS::init_style()
{
  
  BondBPM::init_style();

  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Bond bpm/sls requires ghost atoms store velocity");

}

/* ---------------------------------------------------------------------- */

void BondBPMSLS::settings(int narg, char **arg)
{
...
void BondBPMSLS::write_restart(FILE *fp)
{
...
void BondBPMSLS::read_restart(FILE *fp)
{
...
void BondBPMSLS::write_restart_settings(FILE *fp)
{
...
void BondBPMSLS::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondBPMSLS::single(int type, double rsq, int i, int j, double &fforce)
{
  ...
  // Steady-state solution t->inf
  fforce += (k0[type] * k1)/(k0[type] + k1) * (r0 - r);
  ...
  svector[nhistory] = 0;//fel;
  svector[nhistory + 1] = 0;//fint;
...
void BondBPMSLS::null_table(Table *tb)
{
...
void BondBPMSLS::free_table(Table *tb)
{
...
void BondBPMSLS::read_table(Table *tb, char *file, char *keyword)
{
...
void BondBPMSLS::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->r0 = 0.0;
...
/* ---------------------------------------------------------------------- */
 ...
 void BondBPMSLS::update_table(int type)
{   
  double dt = update->dt;
  double k_temp, eta_temp, exp_j;
  const Table *tb = &tables[tabindex[type]];
...
void BondBPMSLS::bcast_table(Table *tb) // *UPDATED
{
...
void *BondBPMSLS::extract(const char *str, int &dim)
{
  dim = 1;
