...
/* ----------------------------------------------------------------------
   Contributing authors: Zachary White (SNL), Joel Clemmer (SNL)
------------------------------------------------------------------------- */

#include "bond_bpm_pronyplastic.h"
...
BondBPMPronyplastic::BondBPMPronyplastic(LAMMPS *_lmp) :
    BondBPM(_lmp), k0(nullptr), ecrit(nullptr), eplastic(nullptr), gamma(nullptr), 
...
BondBPMPronyplastic::~BondBPMPronyplastic()
{
  delete[] svector;
...
double BondBPMPronyplastic::store_bond(int n, int i, int j)
{
  int type;
  ...
  if (i < atom->nlocal) {
    for (int m = 0; m < atom->num_bond[i]; m++) {
      if (atom->bond_atom[i][m] == tag[j]) { 
        ...
        fix_bond_history->update_atom_value(i, m, 4, 0); // E_cu
        ...
        fix_bond_history->update_atom_value(i, m, l+5, 0);
  ...
  if (j < atom->nlocal) {
    for (int m = 0; m < atom->num_bond[j]; m++) {
      if (atom->bond_atom[j][m] == tag[i]) { 
        ...
        fix_bond_history->update_atom_value(j, m, 4, 0); // E_cu
        ...
        // Internal stress variable
        fix_bond_history->update_atom_value(j, m, l+5, 0);     
...
void BondBPMPronyplastic::store_data()
{
  int i, j, n, m, type;
  double delx, dely, delz, r;
  ...
  for (i = 0; i < atom->nlocal; i++) {
    for (m = 0; m < atom->num_bond[i]; m++) {
      ...
      for (int n = 0; n < tb->ninput; n++ ) {
        ...
        // Internal stress variable
        fix_bond_history->update_atom_value(i, m, n+5, 0);
        bondstore[m][n+5] = 0;
...
void BondBPMPronyplastic::compute(int eflag, int vflag)
{
  ...
  double e, ep, rsq, r, r0, rn , rinv,  smooth, fbond, dot;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    bondstore[n][1] = r; // update bond length in bondstore
    ...
    if (normalize_flag) {
      ...
      fbond = -k0[type] * (r - (1.0 + ep) * r0);
    }

    En_dot = 0; // reset total dissipation rate
    ...
    //printf("post force\n");
    bondstore[n][3]   = En_dot; // store total previous dissipation rate
    
    // update total cumulative dissipated energy
    Ediss_cu = bondstore[n][4];
    Ed = En_dot * dt;
    Ediss_cu += Ed;
    bondstore[n][4] = Ediss_cu;
...
void BondBPMPronyplastic::allocate()
{
  allocated = 1;
...
void BondBPMPronyplastic::coeff(int narg, char **arg)
{
  ...
  double eplastic_one = utils::numeric(FLERR, arg[3], false, lmp);
  ...
  if (temperature_flag) {
    ...
    if (aT_one <= 0) error->all(FLERR,"Temperature shift coefficient must be postive nonzero");
  } else {
    if (iarg != narg) error->all(FLERR,"Incorrect args for bond coefficients");
  }
...
void BondBPMPronyplastic::init_style()
{
...
void BondBPMPronyplastic::write_restart(FILE *fp)
{
...
void BondBPMPronyplastic::read_restart(FILE *fp)
{
...
void BondBPMPronyplastic::write_restart_settings(FILE *fp)
{
...
void BondBPMPronyplastic::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondBPMPronyplastic::single(int type, double rsq, int i, int j, double &fforce)
{
...
void BondBPMPronyplastic::null_table(Table *tb)
{
...
void BondBPMPronyplastic::free_table(Table *tb)
{
...
void BondBPMPronyplastic::read_table(Table *tb, char *file, char *keyword)
{
  ...
  for (int i = 0; i < tb->ninput; i++) {
    line = reader.next_line();
    if (!line)
      error->one(FLERR, "Data missing when parsing table '{}' line {} of {}.", keyword, i + 1,
                 tb->ninput);
    try {
...
void BondBPMPronyplastic::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->r0 = 0.0;
...
/* ---------------------------------------------------------------------- */
 ...
 void BondBPMPronyplastic::update_table(int type)
{   
  double dt = update->dt;
  double k_temp, eta_temp, exp_j, alph_j, term0;
  const Table *tb = &tables[tabindex[type]];
...
/* ----------------------------------------------------------------------
   ...
   this function communicates these values in Table:
     ninput,r0,kfile,etafile,expfile
------------------------------------------------------------------------- */

void BondBPMPronyplastic::bcast_table(Table *tb) // *UPDATED
{
...
void *BondBPMPronyplastic::extract(const char *str, int &dim)
{
  dim = 1;
