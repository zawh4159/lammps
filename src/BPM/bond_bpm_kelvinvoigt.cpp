...
#include "bond_bpm_kelvinvoigt.h"
...
BondBPMKelvinVoigt::BondBPMKelvinVoigt(LAMMPS *_lmp) :
    BondBPM(_lmp), ecrit(nullptr), gamma(nullptr), 
...
BondBPMKelvinVoigt::~BondBPMKelvinVoigt()
{
  delete[] svector;
...
double BondBPMKelvinVoigt::store_bond(int n, int i, int j)
{
  int type;
...
void BondBPMKelvinVoigt::store_data()
{
  int i, j, n, m, type;
  double delx, dely, delz, r;
...
void BondBPMKelvinVoigt::compute(int eflag, int vflag)
{
...
void BondBPMKelvinVoigt::allocate()
{
  allocated = 1;
...
void BondBPMKelvinVoigt::coeff(int narg, char **arg)
{
...
void BondBPMKelvinVoigt::init_style()
{
...
void BondBPMKelvinVoigt::write_restart(FILE *fp)
{
...
void BondBPMKelvinVoigt::read_restart(FILE *fp)
{
...
void BondBPMKelvinVoigt::write_restart_settings(FILE *fp)
{
...
void BondBPMKelvinVoigt::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondBPMKelvinVoigt::single(int type, double rsq, int i, int j, double &fforce)
{
...
void BondBPMKelvinVoigt::null_table(Table *tb)
{
...
void BondBPMKelvinVoigt::free_table(Table *tb)
{
...
void BondBPMKelvinVoigt::read_table(Table *tb, char *file, char *keyword)
{
...
void BondBPMKelvinVoigt::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->r0 = 0.0;
  ...
  tb->ninput = 1; // manually fix to one for KV
...
/* ---------------------------------------------------------------------- */
 ...
 void BondBPMKelvinVoigt::update_table(int type)
{   
  double dt = update->dt;
  double k_temp, eta_temp, exp_j, alph_j, term0;
  const Table *tb = &tables[tabindex[type]];
...
void BondBPMKelvinVoigt::bcast_table(Table *tb) // *UPDATED
{
...
void *BondBPMKelvinVoigt::extract(const char *str, int &dim)
{
  dim = 1;
